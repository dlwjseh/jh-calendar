...
## 단계 체크리스트
- [x] 01 - 테마 모델 (`CalendarTheme` + `.classic`)
- [x] 02 - Environment 주입 (커스텀 `EnvironmentKey`)
- [x] 03 - 뷰 색 교체 (하드코딩 → `theme.xxx`)
- [x] 04 - (선택/추후) 테마 전환 골격 (`@AppStorage` 선택 + 루트 재주입) — 골격 완성. 전환 UI/2번째 테마는 추후.
- [x] 05 - 다크 테마 추가 (첫 테마 적용 사례) → [05-다크-테마-추가.md](05-다크-테마-추가.md)

이번 목표는 **01~03**. 04는 연결점, 05는 그 위에 올린 첫 실제 테마(dark).
