...
## 단계 체크리스트
- [ ] 01 - `eventsByDay` 를 멀티데이까지 확장 (데이터)
- [ ] 02 - 주(week) 행 컨테이너로 분리 (리팩토링)
- [ ] 03 - `GeometryReader` 로 행 너비 측정 + 주 단위 멀티데이 필터링
- [ ] 04 - 행 overlay 로 단색 멀티데이 바 (단일 lane, 한 주 안)
- [ ] 05 - 셀에서 멀티데이 그리기 끄기 (분기)
- [ ] 06 - 멀티데이 바 안 텍스트 + 시간지정 시계 아이콘
- [ ] 07 - 주 경계 토막 (`DateInterval` 교집합)
- [ ] 08 - lane(트랙) 배치 — `assignLanes` 순수 함수
- [ ] 09 - 셀 내부 컨텐츠 밀어내기 (멀티데이 lane 갯수만큼)
- [ ] 10 - `+N` 오버플로우 합산
- [ ] 11 - 좌/우 끝 모서리 캡
