...
**새로 등장** (15-3 에서):
- `.animation(_:value:)` — view 측에서 특정 값 변화에 트랜잭션을 보장하는 modifier. `withAnimation` 만으로는 SwiftData `@Query` 결과 변경이 트랜잭션에 잡히지 않아 `.transition` 이 안 먹는 문제를 해결.
...
- `withAnimation` + `.transition` 협력 → [03-03](../03-사이드바-슬라이드/03-slide%20애니메이션과%20transition.md), [01-04](../01-타이틀바-호버표시/04-페이드%20인%20아웃%20애니메이션.md)
